...
// Get player's wallet
export const handleGetWallet: RequestHandler = asyncHandler(async (req, res) => {
  if (!req.user) {
...
export const handleUpdateWallet: RequestHandler = asyncHandler(async (req, res) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'Authentication required'
  ...
  let { currency, amount, transaction_type = 'transfer', description, gc_amount, sc_amount } = req.body;
  ...
  const transactionDesc = description || `${transaction_type} - update`;
...
export const handleGetTransactions: RequestHandler = asyncHandler(async (req, res) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'Authentication required'
...
export const handleGetWalletStats: RequestHandler = asyncHandler(async (req, res) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'Authentication required'
