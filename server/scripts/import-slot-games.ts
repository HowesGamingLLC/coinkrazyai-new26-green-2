...
// Extracted game data from the provided HTML
const GAMES_TO_ADD = [
  {
...
export async function importSlotGames() {
  ...
  try {
    for (const game of GAMES_TO_ADD) {
      try {
        ...
        if (existingGame.rows.length > 0) {
          console.log(`[GameImport] Game already exists: ${game.title} by ${game.provider}, skipping...`);
          continue;
        }

        // Insert new game
        const result = await query(
          `INSERT INTO games (title, provider, category, image, game_url, rtp, enabled)
    ...
    console.log('[GameImport] Import completed!');
